...
it('lets a user transfer a star', async() => {
    ...
    let owner = await instance.ownerOf(starId, { from: user1 });
    assert.equal(owner, user1);
    // 2. use the transferStar function implemented in the Smart Contract
    await instance.transferStar(user2, starId, { from: user1 });
    // 3. Verify the star owner changed.
    owner = await instance.ownerOf(starId, { from: user1 });
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    const instance = await StarNotary.deployed();
    const starId = 9;
    const starName = 'Start D'
    await instance.createStar(starName, starId, { from: owner });
    // 2. Call your method lookUptokenIdToStarInfo
    const detectedName = await instance.lookUptokenIdToStarInfo(starId, { from: owner });
    // 3. Verify if you Star name is the same
    assert.equal(detectedName, starName);
});